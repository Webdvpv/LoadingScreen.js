...
function loadingscreen(placement, settings) {
    ...
    if (settings != undefined) {
        ...
        /* IMAGE */
        if (Object.keys(settings.image).length != 0 && settings.image.name != undefined && settings.image.name != "" && settings.image.path != undefined && settings.image.path != "") wrapper.innerHTML = `<img src="${settings.image.path + settings.image.name}" alt="${settings.image.alt != undefined || settings.image.alt != "" ? settings.image.alt : ""}">`
        else console.error("All defines must be defined!");
        /* IMAGE END */
